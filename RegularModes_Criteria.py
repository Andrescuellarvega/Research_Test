...
def Periodic_Coefficient_Matrices(frequency_grid, current_k, N_SQUIDs):
    ...
    A_nl_squared = np.abs(A_matrix)**2
    B_nl_squared = np.abs(B_matrix)**2
    C_nl_squared = np.abs(C_matrix)**2
    D_nl_squared = np.abs(D_matrix)**2

    A_nn_sqared = np.diagonal(A_nl_squared)
...
args = parser.parse_args('-nb 1000 -sb 2 -sq 101'.split())
