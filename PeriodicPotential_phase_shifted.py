...
# current_k = Current frequency bin for which matrices are calculated,
# N_SQUIDs = Number of SQUIDs connected in series.
#
...
def Periodic_Coefficient_Matrices(frequency_grid, current_k, N_SQUIDs):
    ...
    # ------------------------------------------------------------------------------------------------------------------
    #   PROPAGATION MATRIX FROM ONE SQUID TO NEXT
    # ------------------------------------------------------------------------------------------------------------------
    ...
    #Trying to do the matrix multiplication with different transfer matrices instead of the same one to a power
    for ith_SQUID in range(1, N_SQUIDs+1):

        for m in range(1, 2*N_sidebands+2):
            for n in range(1, 2*N_sidebands+2):
                G_matrices[ith_SQUID-1, m-1, n-1] = (1/2) * np.complex(0,1) * (vcpw/(ww[m - 1]))*(1/L0_eff) * g(ww, n, m, ith_SQUID)
        ...
        S_hat[ith_SQUID-1, :, :] = np.concatenate((np.concatenate((one_matrix - G_matrices[ith_SQUID-1], -G_matrices[ith_SQUID-1]), axis=1),
                                np.concatenate((G_matrices[ith_SQUID-1], one_matrix + G_matrices[ith_SQUID-1]), axis=1)), axis=0)
...
for k in range(0, freq_grid_size): #Calculate transfer matrices and output for each frequency in the range
